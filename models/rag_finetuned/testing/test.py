#!/usr/bin/env python3
"""
lemma_context_sentiment_eval.py

Evaluate an Ollama Latin sentiment classifier, but add *lemma-matched lexicon context*
...
OLLAMA_MODEL = "latin-sentiment-finetuned"
...
SLEEP_S = 0.2                # throttle between requests (you had 0.2)
...
def ollama_generate(prompt: str, timeout_s: int = 180) -> str:
    payload = {
        ...
        "prompt": prompt,
        "stream": False,
        "options": {"num_predict": 20, "temperature": 0.1, "top_p": 0.9},
    }
...
def fetch_lemma_context(conn: psycopg.Connection, lemmas: List[str]) -> List[Dict[str, Any]]:
    ...
    with conn.cursor() as cur:
        # psycopg3: use ANY(%s) and pass a list; common pattern is wrapping list in another list. [web:29]
        cur.execute(
            """
    ...
    for lemma, pos, polarity_score, has_polarity, provenance in rows:
        ...
        if CONTEXT_MODE == "POLAR_ONLY":
            ...
            if rec["polarity_score"] is None:
                continue
            # If has_polarity is used as a flag column in your DB, keep non-empty.
...
def main():
    load_dotenv()  # typical pattern for loading DATABASE_URL from .env. [web:33]
    ...
    results_csv = OUT_DIR / f"results_lemmactx_{ts}.csv"
    summary_path = OUT_DIR / f"summary_lemmactx_{ts}.json"
