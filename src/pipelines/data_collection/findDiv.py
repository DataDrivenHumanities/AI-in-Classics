...
def findElements(
    root: ET.Element, tTree: ET.ElementTree, index: list, isBody: bool = False
):
    # do whatever you want with root here
    if isBody & (root.text is not None):
        if (
            len(set(root.text) - {" ", "\n", "\t"}) > 0
        ):  # if text contains letters other than tab or space
...
def writeDescrip(
    tTree: ET.ElementTree,
    t_author: list = ["Unknown Author"],
    t_title: str = "Unknown",
    titleTag: str = "",
):
    # add author title
