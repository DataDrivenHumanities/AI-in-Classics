...
def FindEngElem(
    elemList: list = [], element: ET.Element = None, IOI: ET.Element = None
):
    """
    ...
    for event, elem in ET.iterparse(
        BytesIO(ET.tostring(element)), events=("start",)
    ):  # , 'end')):
    ...
    with open("./temp.csv", "w", newline="", encoding="UTF8") as newCsv:
        ...
        with open("./csvTextPartsWithTitleAndAuthor.csv", newline="") as csvIn:
            ...
            for row in reader:
                ...
                count += 1
                if (count % 10000) == 0:
                    print(int(count / 10000), end=" ", flush=True)
                fileAndIndex = row["Section ID"]
                fileAndIndex = (
                    fileAndIndex.replace("Data/", "")
                    .replace(",", "")
                    .replace("_clean", "")
                )
                ...
                foundEngXml = False
                engXml = [
                    i for i in allEngXmls if ((p1 + p2 in i) or (p1 + "." + p2 in i))
                ]
                if len(engXml) == 0:
                ...
                if engXml in ogXmlDict.keys():
                    ...
                    engTree = ET.parse("./translated_xml/" + engXml)
                ...
                if len(theList) == 0:
                    ...
                    for item in elemSkipList:
                        if item in theList[-1]:
                            notEmptyList = False
                if notEmptyList:

                    findEngElemRes = FindEngElem(theList, engTree.getroot()[1][0][0])
                    transText = "None"
                    if findEngElemRes[0] != None:
                        transText = findEngElemRes[0]
                    # If there is no returned text, but there is an IOI, get IOI text
                    elif findEngElemRes[1] != None:
                        transText = ElemText(findEngElemRes[1])
                    writer.writerow(
                        {
                            "": row[""],
                            "Section ID": row["Section ID"],
...
def EngTextSearch(fileAndIndex: str = None):
    ...
    try:
        ...
        return None
    fileAndIndex = (
        fileAndIndex.replace("Data/", "").replace(",", "").replace("_clean", "")
    )
    filename = fileAndIndex.split("xml")[0]
    ogFile = os.path.join(ogXmlDict[filename + "xml"], filename + "xml")
