...
def runDiv(templatePath: str, cleanFolder: str, ogFolder: str, limit: unsignedinteger = -1):
    ...
    for filename in os.listdir(ogFolder):
        ...
        if (os.path.isfile(f) & (i < limit)): 
            ...
            if ((f[-8:-5] in {"grc","lat","mul"}) | (f[-9:-6] in {"grc","lat","mul"})):
                print(f)
                cleanPath = filename[0:-4] + "_clean.xml"
                cleanPath = os.path.join(cleanFolder,cleanPath)
                file = open(cleanPath, "w")
                findDiv.findDiv(templatePath=templatePath, temWritePath=cleanPath, filePath=f)
                i += 1
