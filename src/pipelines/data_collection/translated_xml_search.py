...
def CopyTranslatedXMLs(
    src: str = "../../../../canonical-greekLit/data/",
    dest: str = "./translated_xml/",
    lang: str = "eng",
):
    """
    ...
    for folder in next(os.walk(src))[1]:
        ...
        for subFolder in next(os.walk(p1))[1]:
            ...
            for item in next(os.walk(p2))[2]:
                if True in [i in item for i in skip]:
                    continue
                if item[-3:] == "xml":
                    ...
                    if filenameParts[2][-4:-1] == lang:
                        ...
                        if langXml == None:
                            langXml = item
                        elif int(item[-5]) > int(langXml[-5]):
                            langXml = item
            if langXml:
                tempSplit = langXml.split(".")
                tempSplit = tempSplit[0] + "." + tempSplit[1]
                if (langXml in item for item in os.listdir(dest)):
