...
def clean(path: str, save_dir_path: str = "./"):
    ...
    textparts = doc("div:parent").filter(
        selector=lambda x: pq(this).attr["type"] == "textpart"
    )
    subtypes = set(
        [textparts.eq(index=i).attr["subtype"] for i in np.arange(len(textparts))]
    )
    subtype_top_level = sorted(subtypes)[0]
    textparts = textparts.filter(
        selector=lambda x: pq(this).attr["subtype"] == subtype_top_level
    )
    for index in np.arange(len(textparts)):
        ...
        html_decoded = (
            textpart.html()
            .encode()
            .decode(encoding="unicode_escape")
            .encode(encoding="raw_unicode_escape")
            .decode()
        )
    ...
    textparts = (
        doc_cleaned("div:parent")
        .filter(selector=lambda x: pq(this).attr["subtype"] == subtype_bottom_level)
        .find(selector="p")
    )
    for index in np.arange(len(textparts)):
        # cprint(text='-' * 100, color=line_color)
    ...
    with open(
        file=f'{save_dir_path + path.split("/")[-1][:-4]}_cleaned.xml', mode="w"
    ) as xml_cleaned:
...
def xml_to_text(path: str, save_dir_path: str = "./"):
    ...
    os.makedirs(name=save_dir_path, exist_ok=True)
    with open(
        file=f'{save_dir_path + path.split("/")[-1][:-4]}_text.txt', mode="w"
    ) as text_file:
        text_file.write(txt)
    ...
    pool.starmap(
        ...
        iterable=tqdm.tqdm(
            np.asarray(
                a=list(
                    [
                        (f"./original_xml/{xml_name}", "./cleaned_xml/")
                        for xml_name in os.listdir(path="./original_xml/")
                    ]
                )
            )
        ),
    ...
    pool.starmap(
        ...
        iterable=tqdm.tqdm(
            np.asarray(
                a=list(
                    [
                        (f"./original_xml/{xml_name}", "./full_texts/")
                        for xml_name in os.listdir(path="./original_xml/")
                    ]
                )
            )
