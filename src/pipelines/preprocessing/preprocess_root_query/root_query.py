...
def query_data(input_file, query_word, data_table):
    ...
    ids_from_word = grk_lemmatizer.get_id(
        query_word
    )  # transform word to list of ids with lemmatizer

    data_table = pd.read_csv(
        data_table, delimiter=",", header=0
    ...
    for id_from_word in ids_from_word:
        if id_from_word in dictionary:
            ...
            final_table = pd.concat(
                [final_table, pd.DataFrame(dataframe_row)], ignore_index=True
            )
...
result = query_data(
    "src/pipelines/preprocessing/preprocess_root_query/needed_files/dictionary_grk.pkl",
    "φιλια",
    "src/pipelines/preprocessing/preprocess_root_query/needed_files/0-50kgreekv1.csv",
)
print(result)
