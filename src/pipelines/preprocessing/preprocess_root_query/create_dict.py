...
def init_dict(lang: Language, force=None, limit=None, complete=False):
    ...
    def remove_non_alpha(row):
        ...
        return "".join(
            char for char in s if unicodedata.category(char)[0] in {"L", "M", "Z"}
        )
    ...
    if complete:
        suffix = suffix + "_complete"
    dict_filename = f"dictionary_{suffix}.pkl"

    if os.path.isfile(dict_filename):
        if not force:
            print(
                "Pickled dictionary file already exists. Delete it and rerun or run with --force."
            )
            exit()
