...
def inverted_index_to_stemmed_inverted_index(
    inverted_index: Dict[str, Iterable],
    ...
    for inflected_term in tqdm.tqdm(iterable=inverted_index):
        ...
        stemmed_inverted_index[stem] = stemmed_inverted_index[stem] | {
            inflected_term: inverted_index[inflected_term]
        }
    return stemmed_inverted_index


def stemmed_inverted_index_to_json(
    stemmed_inverted_index: Dict[str, Dict[str, Iterable]], save: bool = False
) -> str:
    stemmed_inverted_index_str = json.dumps(
        obj=stemmed_inverted_index, cls=InvertedIndexJSONEncoder
    )
    if save:
...
try:
    ...
    pickle.dump(
        obj=stemmed_inverted_index,
        file=open(file="stemmed_inverted_index.pkl", mode="wb"),
    )
