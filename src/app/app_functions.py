...
def query_cb():
    ...
    kws = lemmatizer.lemmatize(tokens=list([token for token in cltk_normalize(text=query_input).split()]))
...
def doc_term_matrix():
    ...
    if len(os.listdir(path=PREPROCESSED_TEXTS_PATH)) == 0:
        st.warning(body='WARNING (DOC_TERM_MATRIX): No documents have been preprcessed yet.')
    
    preprocessed_txt_paths = [PREPROCESSED_TEXTS_PATH + file_name for file_name in os.listdir(path=PREPROCESSED_TEXTS_PATH)]
