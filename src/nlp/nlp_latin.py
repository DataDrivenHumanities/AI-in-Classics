...
# region
cprint(text="cltk.nlp.NLP object:", color="green", attrs=["bold", "dark"])
...
def filter_by_lemma(doc: cltk.core.data_types.Doc, lemmas: Iterable[str]):
    ...
    return list(
        [
            token
            for idx, token in enumerate(iterable=doc.tokens)
            if lemmas_in_doc[idx] in lemmas
        ]
    )
...
def filter_by_pos(doc: cltk.core.data_types.Doc, pos_tags: Iterable[str]):
    ...
    return list(
        [
            token
            for idx, pos_tag in enumerate(iterable=doc.pos_tags)
...
def filter_by_weight(
    terms: Iterable[str], weights: Iterable[int], threshold: Union[int, float]
):
    """
    ...
    if len(terms) != len(weights):
        raise ValueError(
            f"The number of terms ({len(terms)}) does not equal the number of weights ({len(weights)})."
        )
